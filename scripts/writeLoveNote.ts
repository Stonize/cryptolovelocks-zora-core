...
async function start() {
  ...
  if (addressBook.media) {
    ...
    console.log('Begin to write for token ' + args.tokenId + ":");
    console.log(args.message)
    // console.log(BigNumber.from(args.gwei).mul(BigNumber.from("1000000000")))

    const media = MediaFactory.connect(addressBook.media, wallet);

    if (args.nonce) {
      const tx = await media.setLoveMessage(args.tokenId, args.message, {
        gasLimit: BigNumber.from("1000000"),
        nonce: parseInt(args.nonce),
        gasPrice: BigNumber.from(args.gwei).mul(BigNumber.from("1000000000"))
      })
      console.log(`Transaction for love note: ${tx.hash}`);
    }
    else {
      const tx = await media.setLoveMessage(args.tokenId, args.message, {
        gasLimit: BigNumber.from("1000000"),
